...
const BaseToast = ({
    ...
    return (
        <div className={cn('toast-container', ...classNames)}>
            {imageSrc && <img src={imageSrc} alt={`${type} icon`} className={cn('toast-icon')}/>}
            <div className={cn('toast-message', `toast-message-${type}`,...classNames)}>
                <p>{message || presetMessage}</p>
                <p className={cn('toast-secondary-message')}>{secondaryMessage || presetSecondaryMessage}</p>
                {children}
            </div>
            {closable && (
